...
import { Search, Filter, BookOpen, AlertCircle, Sparkles, CheckCircle2, ShieldAlert } from 'lucide-react';

function LibraryApp() {
  const { role, getAuthHeaders } = useAuth();
  const [activeTab, setActiveTab] = useState<'catalog' | 'dashboard' | 'audit' | 'users'>('catalog');
  ...
  useEffect(() => {
    fetchBooks();
  }, [searchQuery, selectedFaculty, selectedStatus, role]);
  ...
  const handleDownload = async (book: Book) => {
    try {
      ...
      if (!res.ok) {
        ...
        showToast(
          errJson.error || 'Tài liệu này không cho phép tải về máy theo quy chế thư viện',
          'error'
        );
        return;
      }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-3">
              <div className="flex flex-col md:flex-row gap-3">
                ...
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder="Tìm kiếm tài liệu theo tiêu đề, tác giả, môn học, từ khóa..."
              ...
              <div className="flex items-center justify-between text-xs text-slate-500 pt-1 border-t border-slate-100">
                <span>
                  Hiển thị <strong className="text-slate-900">{books.length}</strong> giáo trình & tài liệu số
                </span>
                <span className="hidden sm:inline text-slate-400">
                  Tài liệu bảo mật định dạng PDF • Tích hợp trình đọc PDF.js & chống sao chép
                </span>
              </div>
            </div>
            ...
            {loading ? (
              <div className="py-20 text-center space-y-3">
                ...
                <p className="text-xs text-slate-500">Đang đồng bộ dữ liệu thư viện...</p>
              </div>
            ) : books.length > 0 ? (
      ...
      <footer className="border-t border-slate-200 bg-white py-6 mt-12 text-center text-xs text-slate-500">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <div>
            <strong>Hệ thống Thư viện Số V1.0</strong> • Trường Cao đẳng Công nghệ & Đào tạo
          </div>
          <div className="text-slate-400">
            PostgreSQL • Drizzle ORM • Express REST API • Bảo mật PDF Stream
          </div>
        </div>
      </footer>
