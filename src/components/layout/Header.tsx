...
import { BookOpen, Shield, UserCheck, GraduationCap, BarChart3, LogIn, LogOut, Users, FileText, ChevronDown } from 'lucide-react';

interface HeaderProps {
  activeTab: 'catalog' | 'dashboard' | 'audit' | 'users';
  onTabChange: (tab: 'catalog' | 'dashboard' | 'audit' | 'users') => void;
  onOpenUpload: () => void;
}

export const Header: React.FC<HeaderProps> = ({ activeTab, onTabChange, onOpenUpload }) => {
          ...
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => onTabChange('catalog')}>
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-blue-100 text-blue-800">V1.0</span>
              </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            {(role === 'librarian' || role === 'admin') && (
              <button
                ...
                className="hidden sm:inline-flex items-center px-3.5 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-blue-600 hover:bg-blue-700 shadow-xs transition-colors"
              >
                + Nhập sách PDF
              </button>
            )}
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg border text-xs font-semibold ${currentBadge.bg} transition-all`}
                title="Nhấn để đổi vai trò kiểm thử hệ thống"
              >
                <CurrentIcon className="w-3.5 h-3.5" />
                <span>Vai trò: {currentBadge.label}</span>
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-slate-200 py-2 z-50 animate-in fade-in zoom-in-95">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-slate-400">Tra cứu, đọc online & tải theo quyền</div>
                    </div>
                  </button>
                  <button
                    onClick={() => {
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-slate-400">Upload PDF, quản lý metadata, duyệt sách</div>
                    </div>
                  </button>
                  <button
                    onClick={() => {
